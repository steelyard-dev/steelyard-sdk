...
export function openLedger(path: string): LedgerDatabase {
...
function migrate(db: LedgerDatabase): void {
  const hasVersionTable = db
    .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'schema_version'")
    .get() as { name: string } | undefined;
  const version = hasVersionTable
    ? (db.prepare("SELECT MAX(version) AS version FROM schema_version").get() as { version: number | null }).version ?? 0
    : 0;
