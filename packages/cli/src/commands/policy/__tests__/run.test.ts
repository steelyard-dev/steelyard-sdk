...
describe("steelyard policy run", () => {
  ...
  it("waits for SIGTERM when no explicit shutdown hook is provided", async () => {
    const dir = mkdtempSync(join(tmpdir(), "policy-run-"));
    const policy = join(dir, "policy.yaml");
    const dataDir = join(dir, "data");
    writeFileSync(policy, POLICY);
    const signals = new AutoStopSignals("SIGTERM");
    const io = captureIo(dir);

    const result = await policyRunCommand({ policy, dataDir, signalSource: signals }, io);
...
class AutoStopSignals extends EventEmitter {
  constructor(private readonly stopSignal: string) {
    super();
  }

  override once(eventName: string | symbol, listener: (...args: any[]) => void): this {
    const result = super.once(eventName, listener);
    if (eventName === this.stopSignal) setImmediate(() => this.emit(eventName));
    return result;
  }
}
