...
`Wallet.purchase(intent, { merchant })` is the merchant checkout surface. It
composes:
...
## What `purchase()` guarantees

`purchase()` re-runs policy before spending. If denied, it throws
...
## Browser-Manual Sessions

Use `wallet.createBrowserManualSession(intent)` only for browser-manual or
browser-legacy checkout. It returns a `BrowserManualSession` that can reveal raw
card data inside `revealCard()`.
