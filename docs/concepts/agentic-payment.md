...
Steelyard v0.7 makes the checkout loop explicit while keeping UCP payment
adapters neutral. Merchants advertise PSP capabilities as UCP payment handlers,
buyers select wallet issuers by instrument type, and the merchant adapter
verifies the resulting payment handle before capture. Stripe SPT remains
supported, and the guarded reference PSP validates the same UCP shape with
`delegated_payment_token` handles.

On UCP, a checkout can run with AP2 payment mandates or direct selected
instruments. In AP2 sessions, the issuer's payment handle is embedded inside
the AP2 payment mandate's existing `payment_instrument` claim, so the merchant
can verify user consent before handing the handle to the PSP. In direct UCP
sessions, the selected instrument carries the issuer's advertised
`instrumentType`.
...
sequenceDiagram
  ...
  participant Issuer as Payment issuer
  ...
  Merchant->>PSP: Verify/capture handle
...
then expose it everywhere. UCP can use adapter-neutral payment capabilities,
