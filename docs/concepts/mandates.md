...
merchant checkout. Steelyard uses mandates for UCP checkout because UCP
completion can otherwise carry a payment credential without a buyer-signed
checkout snapshot.
...
verification. Use `mockMandateVerifier()` only for tests and demos; outside
known test environments it requires both an explicit option and
